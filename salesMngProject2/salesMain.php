...
$perPage = 50; // 한 페이지에 표시할 항목 수
$sortBy = isset($_GET['sort']) ? $_GET['sort'] : 'SALE_ID'; // 기본 정렬 기준

// 해당 페이지의 데이터를 가져오는 SQL 쿼리 작성
$offset = ($page - 1) * $perPage;
$sql = "SELECT * FROM SALES LIMIT $offset, $perPage";
...
function selectSales($sortBy, $receivedSN = null, $receivedSaleID = null, $receivedOrderNo = null) {
    ...
    $sql .= "LEFT JOIN DEVICE AS d ON s.ORDER_NO = d.ORDER_NO "; 
...
function searchFunction($saleId, $vId = "", $cId = "", $cbizId = "", $bizId = "", $deliverDateFrom = "", $deliverDateTo = "", 
                $sDateFrom = "",$sDateTo = "", $dDateFrom="", $dDateTo="", $warranty = "", $orderNo ="", $ref=""){
    $sql =  "SELECT s.SALE_ID as SALE_ID, ";
    $sql .= "v.NAME  as V_NAME, ";
    $sql .= "c.NAME  as C_NAME, ";
    $sql .= "cbiz.NAME as CBIZ_NAME, ";
    ...
    if (!empty($deliverDateFrom)) { $conditions[] = "s.DELIVER_DATE >= '".$deliverDateFrom."'"; }
    if (!empty($deliverDateTo)) { $conditions[] = "s.DELIVER_DATE <= '".$deliverDateTo."'"; }
    if (!empty($sDateFrom)) { $conditions[] = "s.S_DATE >= '".$sDateFrom."'"; }
    if (!empty($sDateTo)) { $conditions[] = "s.S_DATE <= '".$sDateTo."'"; }
...
// case 3. salesSearch.php 통해 유입
    ...
    $message = "조건 검색: ";
    $sql = searchFunction(
        $_POST["saleId"] ?? "", $_POST["vId"] ?? "", $_POST["cId"] ?? "",
        $_POST["cbizId"] ?? "", $_POST["bizId"] ?? "", $_POST["deliverDateFrom"] ?? "",
        $_POST["deliverDateTo"] ?? "", $_POST["sDateFrom"] ?? "", $_POST["sDateTo"] ?? "",
        $_POST["dDateFrom"] ?? "", $_POST["dDateTo"] ?? "", $_POST["WARRANTY"] ?? "",
        $_POST["orderNo"] ?? "", $_POST["REF"] ?? ""
    );
        ...
        <div class="row">
                            ...
                            <td colspan="12">
                                <div id="flush-collapse<?php echo $counter; ?>" class="collapse accor-style">
                                  <table class="detail-table">
                                    ...
                                    <!-- DEVICE 테이블에서 ORDER_NO="20190111-2" 조건을 사용하여 쿼리하면 결과로 두 개의 SN 값이 반환됩니다.
                                    그러나 SALES 테이블을 쿼리할 때 S.SALE_ID를 조건으로 사용하고, DEVICE 테이블과 LEFT JOIN을 사용하여 연결합니다.
                                    그 결과, DEVICE_SN 컬럼의 값은 한 개의 SN 값만 반환될 수 있습니다. 왜냐하면 fetch_assoc() 함수는 결과 세트에서 단 하나의 행만 반환하기 때문입니다.
                                    GROUP_CONCAT: GROUP_CONCAT 함수를 사용하여 여러 SN 값을 하나의 문자열로 연결할 수 있습니다. -->
                                      <?php 
            ...
            <div class="pagination">
              <ul class="pagination-list">
                  <?php
                  $totalPages = ceil($totalCount / $perPage); // 전체 페이지 수 계산
  
                  for ($i = 1; $i <= $totalPages; $i++) {
                      $activeClass = ($i == $page) ? 'active' : ''; // 현재 페이지면 활성화 클래스 추가
