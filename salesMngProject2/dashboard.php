...
// 3. 유상 보증기간 종료예정 리스트(LICENSE 테이블의 D_DATE 날짜가 오늘 날짜로부터 30일 이내인 것)
function get_paidToBeExpired($today, &$d_date_tobe_expired, &$query_paid_tobe_expired = "") {
    ...
    $query_paid_tobe_expired = "SELECT COUNT(*) as cnt FROM LICENSE WHERE TYPE = '유상' AND '$today' <= D_DATE AND D_DATE <= '$d_date_tobe_expired'";
...
// echo $d_date_tobe_expired;

/*
function get_paidToBeExpired($today, &$query_paid_tobe_expired = "") {
    global $dbconnect;
    $d_date_tobe_expired = date("Y-m-d", strtotime("+30 days"));
    $query_paid_tobe_expired = "SELECT COUNT(*) as cnt FROM LICENSE WHERE TYPE = '유상' AND '$today' <= D_DATE AND D_DATE <= '$d_date_tobe_expired'";
    $result_paid_tobe_expired = mysqli_query($dbconnect, $query_paid_tobe_expired);
    ...
    return $paidToBeExpired;
}
$paidToBeExpired = get_paidToBeExpired($today);
*/
...
function get_freeLicenseToBeExpired($today, &$query_free_tobe_expired = "") {
    global $dbconnect;
    $d_date_tobe_expired = date("Y-m-d", strtotime("+30 days"));
    $query_free_tobe_expired = "SELECT COUNT(*) as cnt FROM LICENSE WHERE TYPE = '무상' AND '$today' <= D_DATE AND D_DATE <= '$d_date_tobe_expired'";
...
function get_paidExpired($today, &$query_paid_expired = "") {
    global $dbconnect;
    $query_paid_expired = "SELECT COUNT(*) as cnt FROM LICENSE WHERE TYPE = '유상' AND D_DATE <= '$today'";
    // echo $query_paid_expired;
...
function get_freeExpired($today, &$query_free_expired = "") {
    global $dbconnect;
    $query_free_expired = "SELECT COUNT(*) as cnt FROM LICENSE WHERE TYPE = '무상' AND D_DATE <= '$today'";
...
function get_eos($today, &$query_eos = "") {
    ...
    // 6년 6개월 전 날짜 계산
    $start_date = date("Y-m-d", strtotime($today . " -6 years -6 months"));

    // 8년 전 날짜 계산
    $end_date = date("Y-m-d", strtotime($today . " -8 years"));
