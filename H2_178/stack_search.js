// stack_search.js 192.168.100.178
// GET 방식 - 작동 OK
// console.log('stack_search.js 도달!')

let currentSearchConditions = {};
let totalRowsFiltered = 0; // 필터링된 데이터의 총 수를 저장할 변수

const currentDate = new Date();
const formattedDate = currentDate.toISOString().slice(0, 19).replace('T', ' ');


// 페이지 이동 함수(사용자가 페이지 변경할 때 호출, 해당 페이지의 데이터를 가져오도록 searchWithData 함수 호출)
...
document.querySelector('#stack_search_btn').addEventListener('click', function () {
    ...
    console.log("검색 실행 전 현재 검색 조건 : ", currentSearchConditions);
...
function searchWithData(conditions, page = 1) {
    ...
    const url = `http://192.168.100.178/FDC/work/dev/js/main/stack_search.php?${query}`; 
    ...
    xhr.onload = function () {
        if (this.status === 200) {
            ...
            if (Array.isArray(response.data)) {
                totalRowsFiltered = response.totalRows; // 필터링된 데이터의 총 수 업데이트
                console.log("전체 데이터 수 : ", response.totalRows); //데이터 수 로깅
                displayResults(response.data);
                displayPagination(totalRowsFiltered, page); // 필터링된 데이터의 총 수를 초기화하고 페이지네이션 업데이트
...
function updateLabel(date, label){
    ...
    fetch(`http://192.168.100.178/FDC/work/dev/js/main/stack_label_update.php?${queryString}`, {
...
function createPageItem(pageNumber, text, clickable) {
    ...
    pageLink.addEventListener('click', function (event) {
        console.log("페이지네이션 클릭 시 현재 검색 조건:", currentSearchConditions);
        event.preventDefault();
        if (clickable) {
            // searchWithData(currentSearchConditions, pageNumber);
            goToPage(pageNumber); // 변경된 페이지로 이동
        }
    });
